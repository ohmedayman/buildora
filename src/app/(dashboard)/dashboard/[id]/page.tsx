...
const ELEMENT_CATEGORIES = [
  ...
  { label: 'التفاعل', items: [
    ...
    { type: 'whatsapp', icon: '💬', name: 'واتساب', desc: 'زر واتساب' },
    { type: 'social-links', icon: '🔗', name: 'روابط اجتماعية', desc: 'Social Links' },
  ]},
  { label: 'التجارة', items: [
    { type: 'product-card', icon: '🛍', name: 'منتج', desc: 'بطاقة منتج' },
    { type: 'product-grid', icon: '🏪', name: 'شبكة منتجات', desc: 'عرض منتجات' },
  ]},
  { label: 'جاهز', items: [
...
function getDefault(type: string): any {
  const d: Record<string, any> = {
    ...
    whatsapp: { phone: '+201000000000', message: 'مرحباً! أريد الاستفسار', buttonText: 'تواصل معنا عبر واتساب', color: '#25D366' },
    'social-links': { title: 'تابعنا', links: [{platform:'twitter',url:'#',label:'Twitter'},{platform:'instagram',url:'#',label:'Instagram'},{platform:'facebook',url:'#',label:'Facebook'},{platform:'youtube',url:'#',label:'YouTube'}] },
    'product-card': { name: 'منتج رائع', description: 'وصف المنتج هنا', price: '99', originalPrice: '149', image: '', buttonText: 'أضف للسلة', badge: 'جديد' },
    'product-grid': { title: 'منتجاتنا', products: [{name:'منتج 1',price:'49',image:'',desc:'وصف سريع'},{name:'منتج 2',price:'79',image:'',desc:'وصف سريع'},{name:'منتج 3',price:'99',image:'',desc:'وصف سريع'}] },
...
function getElLabel(type: string): string {
  const map: Record<string, string> = {
    ...
    whatsapp: 'واتساب', 'social-links': 'روابط اجتماعية',
    'product-card': 'منتج', 'product-grid': 'شبكة منتجات',
...
function RenderElement({ el }: { el: Element }) {
  ...
  switch (el.type) {
    ...
    case 'whatsapp':
      return <div style={{ textAlign: 'center', padding: 20 }}><a href={`https://wa.me/${c.phone?.replace(/[^0-9]/g, '') || ''}?text=${encodeURIComponent(c.message || '')}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 32px', background: c.color || '#25D366', color: '#fff', borderRadius: 12, fontSize: 16, fontWeight: 600, textDecoration: 'none', boxShadow: '0 4px 15px rgba(37,211,102,0.3)' }}><span style={{ fontSize: 24 }}>💬</span>{c.buttonText || 'تواصل معنا عبر واتساب'}</a></div>;
    case 'social-links':
      return <div style={{ padding: '40px 20px', textAlign: 'center' }}><h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 20 }}>{c.title}</h3><div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>{(c.links || []).map((l: any, i: number) => <a key={i} href={l.url || '#'} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 20px', background: '#f1f5f9', borderRadius: 8, color: '#475569', fontSize: 14, fontWeight: 600, textDecoration: 'none', transition: 'all 0.2s' }}><span>{l.platform === 'twitter' ? '🐦' : l.platform === 'instagram' ? '📸' : l.platform === 'facebook' ? '📘' : l.platform === 'youtube' ? '🎬' : l.platform === 'linkedin' ? '💼' : '🔗'}</span>{l.label}</a>)}</div></div>;
    case 'product-card':
      return <div style={{ maxWidth: 340, margin: '0 auto', padding: 20 }}><div style={{ border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden', background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>{c.image ? <img src={c.image} alt={c.name} style={{ width: '100%', height: 200, objectFit: 'cover' }} /> : <div style={{ height: 200, background: 'linear-gradient(135deg,#6c63ff,#e91e63)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 48 }}>🛍</div>}<div style={{ padding: 20 }}>{c.badge && <span style={{ display: 'inline-block', background: '#fef3c7', color: '#d97706', padding: '3px 10px', borderRadius: 12, fontSize: 11, fontWeight: 600, marginBottom: 8 }}>{c.badge}</span>}<h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{c.name}</h3><p style={{ fontSize: 13, color: '#64748b', marginBottom: 12 }}>{c.description}</p><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}><span style={{ fontSize: 22, fontWeight: 800, color: '#6c63ff' }}>${c.price}</span>{c.originalPrice && <span style={{ fontSize: 14, color: '#94a3b8', textDecoration: 'line-through' }}>${c.originalPrice}</span>}</div><button style={{ width: '100%', padding: '12px', background: '#6c63ff', color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{c.buttonText || 'أضف للسلة'}</button></div></div></div>;
    case 'product-grid':
      return <div style={{ padding: '60px 20px' }}><h2 style={{ textAlign: 'center', fontSize: 28, fontWeight: 800, marginBottom: 40 }}>{c.title}</h2><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 20, maxWidth: 1000, margin: '0 auto' }}>{(c.products || []).map((p: any, i: number) => <div key={i} style={{ border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden', background: '#fff' }}>{p.image ? <img src={p.image} alt={p.name} style={{ width: '100%', height: 160, objectFit: 'cover' }} /> : <div style={{ height: 160, background: 'linear-gradient(135deg,#06b6d4,#3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 36 }}>🛍</div>}<div style={{ padding: 16 }}><h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>{p.name}</h3><p style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}>{p.desc}</p><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontSize: 18, fontWeight: 800, color: '#6c63ff' }}>${p.price}</span><button style={{ padding: '6px 14px', background: '#6c63ff', color: '#fff', border: 'none', borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>أضف</button></div></div></div>)}</div></div>;
    case 'hero':
...
function StylePanel({ element, updateStyle, updateContent }: { element: Element; updateStyle: (s: Partial<ElStyle>) => void; updateContent: (c: any) => void; }) {
  ...
  return (
    <div className="relative">
      ...
      {element.type === 'whatsapp' && <Section title="واتساب"><Input label="رقم الهاتف" value={c.phone} onChange={(v: string) => updateContent({...c, phone: v})} placeholder="+201000000000" /><Input label="الرسالة" value={c.message} onChange={(v: string) => updateContent({...c, message: v})} /><Input label="نص الزر" value={c.buttonText} onChange={(v: string) => updateContent({...c, buttonText: v})} /><Input label="اللون" value={c.color} onChange={(v: string) => updateContent({...c, color: v})} type="color" /></Section>}
      {element.type === 'social-links' && <Section title="روابط اجتماعية"><Input label="العنوان" value={c.title} onChange={(v: string) => updateContent({...c, title: v})} />{(c.links || []).map((l: any, i: number) => <div key={i} className="border-t border-gray-100 pt-2 mt-2"><div className="flex items-center justify-between mb-1"><span className="text-[10px] text-gray-400">الرابط {i+1}</span><button onClick={() => { const links = [...c.links]; links.splice(i,1); updateContent({...c, links}); }} className="text-[10px] text-red-400">✕</button></div><div className="flex gap-1 mb-1">{['twitter','instagram','facebook','youtube','linkedin','tiktok'].map(p => <button key={p} onClick={() => { const links = [...c.links]; links[i] = {...links[i], platform: p}; updateContent({...c, links}); }} className={`px-2 py-0.5 rounded text-[9px] font-semibold border ${l.platform === p ? 'bg-[#6c63ff] text-white border-[#6c63ff]' : 'border-gray-200 text-gray-500'}`}>{p}</button>)}</div><Input label="الرابط" value={l.url} onChange={(v: string) => { const links = [...c.links]; links[i] = {...links[i], url: v}; updateContent({...c, links}); }} /><Input label="التسمية" value={l.label} onChange={(v: string) => { const links = [...c.links]; links[i] = {...links[i], label: v}; updateContent({...c, links}); }} /></div>)}<button onClick={() => updateContent({...c, links: [...(c.links || []), {platform:'twitter',url:'#',label:'Twitter'}]})} className="w-full py-1.5 border border-dashed border-gray-300 rounded-lg text-[10px] text-gray-400 hover:border-[#6c63ff] hover:text-[#6c63ff]">+ إضافة رابط</button></Section>}
      {element.type === 'product-card' && <Section title="المنتج"><Input label="اسم المنتج" value={c.name} onChange={(v: string) => updateContent({...c, name: v})} /><Input label="الوصف" value={c.description} onChange={(v: string) => updateContent({...c, description: v})} /><Input label="السعر" value={c.price} onChange={(v: string) => updateContent({...c, price: v})} /><Input label="السعر الأصلي" value={c.originalPrice} onChange={(v: string) => updateContent({...c, originalPrice: v})} /><Input label="رابط الصورة" value={c.image} onChange={(v: string) => updateContent({...c, image: v})} /><Input label="نص الزر" value={c.buttonText} onChange={(v: string) => updateContent({...c, buttonText: v})} /><Input label="الشارة" value={c.badge} onChange={(v: string) => updateContent({...c, badge: v})} /></Section>}
      {element.type === 'product-grid' && <Section title="شبكة المنتجات"><Input label="العنوان" value={c.title} onChange={(v: string) => updateContent({...c, title: v})} />{(c.products || []).map((p: any, i: number) => <div key={i} className="border-t border-gray-100 pt-2 mt-2"><div className="flex items-center justify-between mb-1"><span className="text-[10px] text-gray-400">المنتج {i+1}</span><button onClick={() => { const products = [...c.products]; products.splice(i,1); updateContent({...c, products}); }} className="text-[10px] text-red-400">✕</button></div><Input label="الاسم" value={p.name} onChange={(v: string) => { const products = [...c.products]; products[i] = {...products[i], name: v}; updateContent({...c, products}); }} /><Input label="الوصف" value={p.desc} onChange={(v: string) => { const products = [...c.products]; products[i] = {...products[i], desc: v}; updateContent({...c, products}); }} /><Input label="السعر" value={p.price} onChange={(v: string) => { const products = [...c.products]; products[i] = {...products[i], price: v}; updateContent({...c, products}); }} /><Input label="رابط الصورة" value={p.image} onChange={(v: string) => { const products = [...c.products]; products[i] = {...products[i], image: v}; updateContent({...c, products}); }} /></div>)}<button onClick={() => updateContent({...c, products: [...(c.products || []), {name:'منتج جديد',price:'0',image:'',desc:'وصف'}]})} className="w-full py-1.5 border border-dashed border-gray-300 rounded-lg text-[10px] text-gray-400 hover:border-[#6c63ff] hover:text-[#6c63ff]">+ إضافة منتج</button></Section>}
